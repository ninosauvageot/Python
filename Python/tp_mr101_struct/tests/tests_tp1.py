...
class TestTP1(unittest.TestCase):
    ...
    def test_compter(self):
        ...
        for j in range(1000):
            v = randint(-10, 10)
            n = compter(lst, v)
            self.assertEqual(lst.count(v), n, f"Le nombre d'occurrences de {v} dans {lst} devrait être {lst.count(v)} au lieu de {n}")
    ...
    def test_nthIndexOf(self):
        for i in range(1000):
            ...
            for j in range(100):
                ...
                self.assertEqual(n2, n1,
                                 f"{n}ème position de {v} dans {lst} devrait retourner {n2} au lieu de {n1}.")
